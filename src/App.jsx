...
import {
  ...
  RotateCcw, Eye, Filter, Calendar, BarChart2, Settings,
} from "lucide-react"
...
const COR = {
  incolor: { label: "Incolor", bg: "#EFF6FF", dot: "#60A5FA", piece: "#DBEAFE", stroke: "#3B82F6", scrap: "#93C5FD" },
  verde:   { label: "Verde",   bg: "#F0FDF4", dot: "#22C55E", piece: "#DCFCE7", stroke: "#16A34A", scrap: "#86EFAC" },
  fume:    { label: "Fume",    bg: "#1F2937", dot: "#9CA3AF", piece: "#374151", stroke: "#6B7280", scrap: "#4B5563" },
}
...
const dayKey = (ts) => new Date(ts).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" })
...
function buildSheet(placed, freeRects, W, H, extra) {
  ...
  const allScraps = freeRects.filter(r => r.w >= 10 && r.h >= 10)
  return {
    width: W, height: H, pieces: placed,
    freeRects: allScraps,
    scraps, // significant scraps for display
...
// Otimizacao completa: chapas primeiro, retalhos depois
// Usa o MENOR retalho que cabe na peca (economiza os maiores para pecas maiores)
function runFullOptimization(pecas, chapas, retalhos, cor) {
  ...
  if (chapaDaCor) {
    ...
    toPlace = [] // todas as pecas foram para chapas
  }

  // 2. RETALHOS POR ULTIMO (menor retalho que cabe - economiza grandes para pecas grandes)
  const retalhosDaCor = retalhos
    .filter(r => r.status === "ativo" && r.cor === cor)
    .sort((a, b) => a.largura * a.altura - b.largura * b.altura) // MENOR primeiro
...
// ══════════════════════════════════════════════════════════
// SVG DA CHAPA - estilo profissional igual imagem referencia
// ══════════════════════════════════════════════════════════
function SheetSVG({ sheet, maxW, cor }) {
  const w = maxW || 500
  const scaleX = w / sheet.width
  const svgH = Math.min(sheet.height * scaleX, 520)
  const scaleY = svgH / sheet.height
  const sx = v => Math.round(v * scaleX)
  const sy = v => Math.round(v * scaleY)
  const pieceColor = COR[cor]?.piece || "#DCFCE7"
  const pieceStroke = COR[cor]?.stroke || "#16A34A"
  const scrapColor = COR[cor]?.scrap || "#93C5FD"

  // Numerar retalhos significativos
  const sigScraps = sheet.freeRects.filter(r => r.w >= 80 && r.h >= 80)
    .sort((a, b) => b.w * b.h - a.w * a.h)

  return (
    <div style={{ position: "relative" }}>
      {/* Header da chapa */}
      <div style={{ background: "#1A261A", padding: "8px 14px", borderRadius: "8px 8px 0 0", display: "flex", alignItems: "center", gap: 8 }}>
        <div style={{ width: 10, height: 10, borderRadius: 2, background: T.green }} />
        <span style={{ fontSize: 12, color: "#9CA3AF", fontFamily: "monospace" }}>
          MAPA DE CORTE · {sheet.width} x {sheet.height} mm · Aproveit: <span style={{ color: T.green, fontWeight: 700 }}>{sheet.efficiency}%</span>
        </span>
      </div>

      <svg width={w} height={svgH} style={{ display: "block", background: "#1A2A1A" }}>
        {/* Fundo */}
        <rect width={w} height={svgH} fill="#1A2A1A" />

        {/* Grid sutil */}
        {Array.from({ length: 20 }, (_, i) => (
          <line key={"gv" + i} x1={Math.round(w * i / 20)} y1={0} x2={Math.round(w * i / 20)} y2={svgH}
            stroke="#1F2F1F" strokeWidth={0.5} />
        ))}
        {Array.from({ length: 15 }, (_, i) => (
          <line key={"gh" + i} x1={0} y1={Math.round(svgH * i / 15)} x2={w} y2={Math.round(svgH * i / 15)}
            stroke="#1F2F1F" strokeWidth={0.5} />
        ))}
          ...
          return (
            <g key={"f" + i}>
              <rect x={sx(r.x)} y={sy(r.y)} width={rw} height={rh}
                fill={isSig ? scrapColor + "33" : "#2A3A2A"}
                stroke={isSig ? scrapColor : "#3A4A3A"}
                strokeWidth={isSig ? 1.5 : 0.8}
                strokeDasharray={isSig ? "6 3" : "3 3"}
                opacity={0.9} />
              {isSig && rw > 60 && rh > 40 && (
                <>
                  <text x={sx(r.x) + rw / 2} y={sy(r.y) + rh / 2 - 10}
                    textAnchor="middle" dominantBaseline="middle"
                    fontSize={Math.max(9, Math.min(13, rw * 0.1))} fill={scrapColor} fontFamily="monospace" fontWeight="700">
                    Retalho {scrapIdx + 1}
                  </text>
                  <text x={sx(r.x) + rw / 2} y={sy(r.y) + rh / 2 + 4}
                    textAnchor="middle" dominantBaseline="middle"
                    fontSize={Math.max(8, Math.min(11, rw * 0.085))} fill={scrapColor} fontFamily="monospace">
                    {Math.round(r.w)} x {Math.round(r.h)}
                  </text>
                  <text x={sx(r.x) + rw / 2} y={sy(r.y) + rh / 2 + 18}
                    textAnchor="middle" dominantBaseline="middle"
                    fontSize={Math.max(7, Math.min(10, rw * 0.075))} fill={scrapColor + "BB"} fontFamily="monospace">
                    {area_m2(r.w, r.h)} m²
                  </text>
                </>
              )}
        ...
        {sheet.pieces.map((p, i) => {
          ...
          const showLabel = pw > 60 && ph > 40
          const fsBig = Math.max(9, Math.min(14, Math.min(pw, ph) * 0.15))
          const fsSmall = Math.max(7, fsBig * 0.78)
          ...
          return (
            <g key={"p" + i}>
              ...
              <rect x={sx(p.x) + 1} y={sy(p.y) + 1} width={pw - 2} height={ph - 2}
                fill={pieceColor} stroke={pieceStroke} strokeWidth={1.5} rx={2} />
              {/* Linha de separacao sutil */}
              <rect x={sx(p.x) + 1} y={sy(p.y) + 1} width={pw - 2} height={ph - 2}
                fill="none" stroke={pieceStroke + "44"} strokeWidth={0.5} rx={2} />
              {showLabel && (
                <>
                  {/* Tamanho de CORTE (com acrescimo) - principal */}
                  <text x={midX} y={midY - (ph > 55 ? fsSmall + 3 : 0)}
                    textAnchor="middle" dominantBaseline="middle"
                    fontSize={fsBig} fill={T.greenDark} fontFamily="monospace" fontWeight="800">
                    {p.pw} x {p.ph}
                  </text>
                  {/* Tamanho FINAL (sem acrescimo) - secundario */}
                  {ph > 55 && (
                    <text x={midX} y={midY + fsBig - 2}
                      textAnchor="middle" dominantBaseline="middle"
                      fontSize={fsSmall} fill={T.greenDark + "AA"} fontFamily="monospace">
                      ({origW} x {origH})
                    </text>
                  )}
                  {/* Indicador rotacao */}
                  {p.rotated && ph > 70 && (
                    <text x={midX} y={midY + fsBig + fsSmall + 2}
                      textAnchor="middle" dominantBaseline="middle"
                      fontSize={Math.max(7, fsSmall * 0.9)} fill={T.green + "AA"}>↺ girada</text>
                  )}
                </>
              )}
              {/* Numero da peca (canto) */}
              {pw > 30 && ph > 22 && (
                <text x={sx(p.x) + 5} y={sy(p.y) + 12}
                  fontSize={9} fill={T.greenDark + "99"} fontFamily="monospace" fontWeight="600">
                  {i + 1}
                </text>
              )}
            </g>
        ...
        {/* Dimensao inferior */}
        <line x1={8} y1={svgH - 12} x2={w - 8} y2={svgH - 12} stroke="#4B5563" strokeWidth={1} markerEnd="url(#arrowR)" markerStart="url(#arrowL)" />
        <text x={w / 2} y={svgH - 4} textAnchor="middle" fontSize={9} fill="#6B7280" fontFamily="monospace">
          {sheet.width} mm
        </text>
        {/* Dimensao lateral */}
        <line x1={8} y1={8} x2={8} y2={svgH - 20} stroke="#4B5563" strokeWidth={1} />
        <text x={16} y={svgH / 2} fontSize={9} fill="#6B7280" fontFamily="monospace" transform={"rotate(-90, 16, " + (svgH / 2) + ")"} textAnchor="middle">
          {sheet.height} mm
        </text>
      </svg>

      {/* Legenda */}
      <div style={{ background: "#141A14", padding: "8px 14px", borderRadius: "0 0 8px 8px", display: "flex", gap: 20, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#9CA3AF" }}>
          <div style={{ width: 14, height: 10, background: pieceColor, border: "1.5px solid " + pieceStroke, borderRadius: 2 }} />
          Peca cortada
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#9CA3AF" }}>
          <div style={{ width: 14, height: 10, background: scrapColor + "33", border: "1.5px dashed " + scrapColor, borderRadius: 2 }} />
          Retalho reaproveitavel
        </div>
        {sigScraps.length > 0 && (
          <div style={{ fontSize: 11, color: scrapColor, fontWeight: 600 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 14 }}>
        ...
        <StatCard icon={<Scissors size={18} />} label="Retalhos uteis" value={retalhoAtivos} sub="disponiveis para uso" accent="#60A5FA" />
        <StatCard icon={<TrendingUp size={18} />} label="Metragem hoje" value={metragemHoje + " m²"} sub="area otimizada hoje" accent={T.green} />
        <StatCard icon={<BarChart2 size={18} />} label="Otimizacoes hoje" value={otimizacoesHoje} sub="realizadas hoje" accent={T.amber} />
      </div>
      ...
      <div style={{ background: T.dark, borderRadius: 18, padding: "20px 20px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>Producao de vidro</div>
        ...
        <ResponsiveContainer width="100%" height={130}>
          <BarChart data={chartData} barSize={chartFilter === "semanal" ? 28 : 12}>
            <XAxis dataKey="d" tick={{ fill: "#9CA3AF", fontSize: 9 }} axisLine={false} tickLine={false}
              interval={chartFilter === "mensal" ? 4 : 0} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <div style={{ background: T.card, borderRadius: 16, padding: 18, boxShadow: "0 1px 6px rgba(0,0,0,0.08)" }}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>Ultimas otimizacoes</div>
          ...
          {lowStock.map(c => (
            <div key={c.id} style={{ background: T.amberLight, border: "1px solid #FCD34D", borderRadius: 12, padding: "12px 16px", marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: T.textMid }}>Chapa {COR[c.cor]?.label} {c.largura}x{c.altura}</div>
    ...
    <div>
      ...
      {showAdd && (
        <div style={{ background: T.card, border: "1px solid " + T.border, borderRadius: 16, padding: 22, marginBottom: 22, boxShadow: "0 4px 24px rgba(0,0,0,0.1)" }}>
          ...
          <Btn onClick={handleAdd} fullWidth disabled={saving}
            icon={saving ? <RefreshCw size={15} style={{ animation: "spin 1s linear infinite" }} /> : <Check size={15} />}>
          ...
          <div key={cor} style={{ marginBottom: 26 }}>
            ...
            {items.map(c => (
              <div key={c.id} style={{ background: T.card, borderRadius: 14, padding: "14px 18px", marginBottom: 8, display: "flex", alignItems: "center", gap: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 17, fontWeight: 700, fontFamily: "monospace" }}>{c.largura} x {c.altura} mm</div>
...
function NewOptimization({ data, setData, navigate, pecasPreenchidas }) {
  ...
  const [pecas, setPecas] = useState(
    pecasPreenchidas ? pecasPreenchidas.pecas : [{ id: uid(), largura: "", altura: "", quantidade: "" }]
  )
  ...
  const handleOptimize = () => {
    ...
    if (!valid.length) { setError("Adicione ao menos uma peca com medidas e quantidade validas."); return }
  ...
  const handleFinalize = async () => {
    ...
    const novaOtm = {
      id, cor, chapas_usadas: chapasNaoRet.length,
    ...
    const pecasParaSalvar = pecas.filter(p => int(p.largura) && int(p.altura) && int(p.quantidade))
    ...
    try {
      const chapaDaCor = data.chapas.filter(c => c.cor === cor && c.quantidade > 0)
        .reduce((best, c) => (!best || c.largura * c.altura > best.largura * best.altura) ? c : best, null)
      if (chapaDaCor && chapasNaoRet.length > 0) {
        await DB.chapas.update(chapaDaCor.id, { quantidade: Math.max(0, chapaDaCor.quantidade - chapasNaoRet.length) })
      }
      ...
      setData(d => ({ ...d, chapas: novasChapas, retalhos: novosRet, otimizacoes: [otmLocal, ...d.otimizacoes] }))
      navigate("history")
  ...
  return (
    <div>
        ...
        {STEPS.map((s, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", flex: i < STEPS.length - 1 ? 1 : "none" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: i + 1 < step ? T.green : i + 1 === step ? T.green : T.border, color: i + 1 <= step ? "#fff" : T.textMuted, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
        ...
        <div>
          ...
          <div style={{ background: T.greenLight, borderRadius: 12, padding: "12px 16px", marginBottom: 24, display: "flex", gap: 10 }}>
            ...
            <div style={{ fontSize: 13, color: T.greenDark }}>
              <strong>+2mm por lado (+4mm total)</strong>. Chapas sao otimizadas primeiro, retalhos por ultimo.
            </div>
          </div>
        ...
        <div>
            ...
            <div key={p.id} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 90px 36px", gap: 8, marginBottom: 8, alignItems: "center" }}>
              {["largura", "altura"].map(field => (
                <input key={field} type="number" value={p[field]}
                  onChange={e => updatePeca(p.id, field, e.target.value)}
              ...
              <input type="number" value={p.quantidade} min={1}
                onChange={e => updatePeca(p.id, "quantidade", e.target.value)}
          ...
          <div style={{ background: T.greenLight, borderRadius: 12, padding: "14px 18px", marginBottom: 20 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              <div><div style={{ fontSize: 11, color: T.textMuted }}>Total de pecas</div><div style={{ fontSize: 22, fontWeight: 800, color: T.greenDark }}>{totalPecas}</div></div>
              <div><div style={{ fontSize: 11, color: T.textMuted }}>Area total</div><div style={{ fontSize: 20, fontWeight: 800, color: T.greenDark }}>{(areaTotal / 1e6).toFixed(2)} m²</div></div>
              <div><div style={{ fontSize: 11, color: T.textMuted }}>Acrescimo</div><div style={{ fontSize: 14, fontWeight: 700, color: T.green }}>+4mm/peca</div></div>
            </div>
          </div>
        ...
        <div>
          ...
          <div style={{ background: T.card, borderRadius: 16, padding: 20, marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {[["Cor", <Pill cor={cor} />], ["Pecas", <span style={{ fontSize: 18, fontWeight: 800 }}>{totalPecas}</span>], ["Area total", <span style={{ fontSize: 16, fontWeight: 700 }}>{(areaTotal / 1e6).toFixed(2)} m²</span>], ["Acrescimo", <span style={{ fontSize: 14, fontWeight: 700, color: T.green }}>+4mm/peca</span>]].map(([l, v], i) => (
          ...
          <div style={{ background: T.card, borderRadius: 16, padding: 20, marginBottom: 20, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
            <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>Pecas a otimizar</div>
      ...
      {step === 4 && result && (() => {
        ...
        const totalPecasDist = result.reduce((t, sh) => t + sh.pieces.length, 0)
        ...
        return (
          <div>
            <div style={{ background: T.greenLight, borderRadius: 14, padding: "14px 18px", marginBottom: 18, display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: T.greenDark }}>
                  {qtdChapas > 0 && qtdChapas + " chapa(s) nova(s) · "}
                  {qtdRet > 0 && qtdRet + " retalho(s) · "}
                  {totalPecasDist} pecas distribuidas
                </div>
              </div>
            </div>
            ...
            {/* SVG */}
            <div style={{ marginBottom: 16, borderRadius: 10, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.15)" }}>
              <SheetSVG sheet={s} cor={cor}
                maxW={typeof window !== "undefined" ? Math.min(600, window.innerWidth - 80) : 500} />
            </div>
            ...
            <div style={{ background: T.card, borderRadius: 14, padding: 16, marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <div style={{ fontSize: 14, fontWeight: 700 }}>
                  {s.isRetalho ? "Retalho " + s.width + "x" + s.height + " mm" : ("Chapa " + (currentSheet + 1) + " de " + result.length + " · " + s.width + "x" + s.height + " mm")}
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, textAlign: "center" }}>
                {[["Pecas", s.pieces.length, T.green], ["Aproveit.", s.efficiency + "%", T.green], ["Desperdicio", (100 - s.efficiency).toFixed(1) + "%", T.red]].map(([l, v, c]) => (
                  <div key={l}>
            ...
            {/* Retalhos gerados */}
            {s.scraps && s.scraps.length > 0 && (
              <div style={{ background: T.card, borderRadius: 14, padding: 16, marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>Retalhos gerados nesta chapa</div>
                {s.scraps.sort((a, b) => b.w * b.h - a.w * a.h).slice(0, 3).map((r, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, paddingBottom: 10, borderBottom: "1px solid " + T.border, marginBottom: 10 }}>
                    <div style={{ width: 36, height: 44, background: COR[cor]?.scrap + "33" || "#93C5FD33", border: "1.5px dashed " + (COR[cor]?.scrap || "#93C5FD"), borderRadius: 4, flexShrink: 0 }} />
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 700, fontFamily: "monospace" }}>Retalho {i + 1}: {Math.round(r.w)} x {Math.round(r.h)} mm</div>
                      <div style={{ fontSize: 12, color: T.textMuted }}>Area: {area_m2(r.w, r.h)} m² · Salvo automaticamente</div>
                    </div>
                  </div>
                ))}
              </div>
...
function HistoryDetail({ otimizacao, onVoltar, onReutilizar }) {
  ...
  useEffect(() => {
    DB.pecas.getByOtimizacao(otimizacao.id)
      .then(d => { setPecas(d); setLoading(false) })
    ...
    <div>
      ...
      <div style={{ background: T.dark, borderRadius: 16, padding: 20, marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {[["Aproveitamento", otimizacao.aproveitamento + "%", T.green], ["Desperdicio", otimizacao.desperdicio + "%", "#FCA5A5"], ["Chapas", otimizacao.chapas_usadas || otimizacao.chapasUsadas, "#fff"], ["Pecas", otimizacao.pecas_totais || otimizacao.pecasTotais, "#fff"], ["Area total", (otimizacao.area_total || otimizacao.areaTotal) + " m²", "#9CA3AF"]].map(([l, v, c]) => (
      ...
      <div style={{ background: T.card, borderRadius: 16, padding: 20, marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
        ...
        ) : <div style={{ textAlign: "center", color: T.textMuted, padding: 20, fontSize: 13 }}>Pecas nao disponiveis.</div>}
      </div>
...
function HistoryScreen({ data, navigate, onReutilizar }) {
  ...
  return (
    <div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                ...
                <div><div style={{ fontSize: 10, color: T.textMuted }}>Pecas</div><div style={{ fontSize: 18, fontWeight: 800 }}>{o.pecas_totais || o.pecasTotais}</div></div>
              </div>
      ...
      {!data.otimizacoes.length && <div style={{ textAlign: "center", padding: "48px 24px", color: T.textMuted }}><Clock size={40} style={{ marginBottom: 12, opacity: 0.4 }} /><div style={{ fontSize: 15, fontWeight: 600 }}>Nenhuma otimizacao ainda</div></div>}
    </div>
  )
}
...
function ScrapsScreen({ data, setData }) {
  ...
  const deleteRetalho = async (id) => {
    if (!confirm("Deletar permanentemente este retalho? Esta acao nao pode ser desfeita.")) return
    try { await DB.retalhos.delete(id); setData(d => ({ ...d, retalhos: d.retalhos.filter(r => r.id !== id) })) }
    catch (e) { alert("Erro: " + e.message) }
  }

  function RetalhoCard({ r }) {
    const c = COR[r.cor] || { piece: "#DCFCE7", stroke: T.green, scrap: "#86EFAC" }
    return (
      <div style={{ background: T.card, borderRadius: 14, padding: "14px 18px", marginBottom: 8, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <div style={{ width: 42, height: 52, background: c.scrap + "33", border: "2px dashed " + c.scrap, borderRadius: 6, flexShrink: 0 }} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "monospace" }}>{r.largura} x {r.altura} mm</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4 }}>
              <Pill cor={r.cor} />
              <span style={{ fontSize: 12, color: T.textMuted }}>{r.area} m²</span>
            </div>
            ...
            {r.status === "descartado" ? (
              <>
                <button onClick={() => updateStatus(r.id, "ativo")}
                  style={{ background: T.greenLight, border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer", display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: T.greenDark, fontWeight: 600, whiteSpace: "nowrap" }}>
                  <RotateCcw size={12} /> Restaurar
                </button>
                <button onClick={() => deleteRetalho(r.id)}
                  style={{ background: T.redLight, border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer", display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: T.red, fontWeight: 600, whiteSpace: "nowrap" }}>
  ...
  return (
    <div>
      ...
      <div style={{ background: T.card, borderRadius: 16, padding: "14px 18px", marginBottom: 20, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, textAlign: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
        ...
        <div><div style={{ fontSize: 18, fontWeight: 800, color: T.textMid }}>{ativos.reduce((s, r) => s + r.area, 0).toFixed(2)} m²</div><div style={{ fontSize: 11, color: T.textMuted }}>Area ativa</div></div>
      </div>
...
// ══════════════════════════════════════════════════════════
// APP SHELL — RESPONSIVO PC + MOBILE
// ══════════════════════════════════════════════════════════
export default function App() {
  ...
  const NAV = [
    ...
    { id: "new-opt",   icon: <Plus size={20} />, label: "Nova Otimizacao" },
  ...
  if (dbError) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: T.dark, flexDirection: "column", gap: 16, padding: 32 }}>
      ...
      <div style={{ color: "#fff", fontSize: 18, fontWeight: 700 }}>Erro de conexao com o banco</div>
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: T.bg, fontFamily: "'Barlow', sans-serif" }}>
        ...
        <div style={{ width: 240, background: T.sidebar, display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, height: "100vh", zIndex: 50, borderRight: "1px solid #1F2D1F" }}>
          ...
          <div style={{ padding: "24px 20px 20px", borderBottom: "1px solid #1F2D1F" }}>
            <div style={{ fontSize: 24, fontWeight: 800, color: "#fff", letterSpacing: -0.5, fontFamily: "'Barlow Condensed', sans-serif" }}>
            ...
            {NAV.map(n => {
              ...
              return (
                <button key={n.id} onClick={() => handleNavigate(n.id)}
                  ...
                  {isNew ? (
                    <div style={{ width: 30, height: 30, borderRadius: 8, background: T.green, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          ...
          <div style={{ padding: "16px 20px", borderTop: "1px solid #1F2D1F" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                <div style={{ color: "#fff", fontSize: 13, fontWeight: 600 }}>Operador</div>
                <div style={{ color: "#4ADE80", fontSize: 10 }}>Online</div>
              </div>
            </div>
            <div style={{ fontSize: 10, color: "#374151", marginTop: 12 }}>OTMglass v1.3.0</div>
          </div>
        </div>
        ...
        <style>{`
          * { box-sizing: border-box; }
          body { margin: 0; background: ${T.bg}; font-family: 'Barlow', sans-serif; }
    ...
    <div style={{ maxWidth: 560, margin: "0 auto", minHeight: "100vh", background: T.bg, fontFamily: "'Barlow', sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: T.dark, padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {screen !== "dashboard" && <ArrowLeft size={20} color="#9CA3AF" style={{ cursor: "pointer" }} onClick={() => handleNavigate("dashboard")} />}
