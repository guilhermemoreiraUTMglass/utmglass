...
import {
  ...
  TrendingUp, Package, Info, CheckCircle, MoreVertical, ChevronLeft, RotateCcw,
} from "lucide-react"
...
const DB = {
  async loadAll() {
    const [{ data: chapas, error: e1 }, { data: retalhos, error: e2 }, { data: otimizacoes, error: e3 }] =
      await Promise.all([
    ...
    if (e1 || e2 || e3) console.error("DB.loadAll errors:", e1, e2, e3)
    return { chapas: chapas || [], retalhos: retalhos || [], otimizacoes: otimizacoes || [] }
  },

  chapas: {
    async insert(row) {
      const { error } = await supabase.from("chapas").insert({ ...row, criado_em: Date.now() })
      if (error) throw error
    },
    async update(id, fields) {
      const { error } = await supabase.from("chapas").update(fields).eq("id", id)
      if (error) throw error
    },
    async delete(id) {
  ...
  retalhos: {
    ...
    async update(id, fields) {
      const { error } = await supabase.from("retalhos").update(fields).eq("id", id)
      if (error) throw error
    },
  },
  ...
  otimizacoes: {
    async insert(row) {
      const { error } = await supabase.from("otimizacoes").insert({ ...row, criado_em: Date.now() })
      if (error) throw error
    },
  },
}
...
// ══════════════════════════════════════════════════════════
// ALGORITMO MAXRECTS — padrão industrial de bin packing
// ══════════════════════════════════════════════════════════
  ...
  for (const piece of pieces) {
    ...
    for (const rect of freeRects) {
      if (pw <= rect.w && ph <= rect.h) {
        ...
        if (s1 < bestScore1 || (s1 === bestScore1 && s2 < bestScore2)) { bestScore1 = s1; bestScore2 = s2; bestRect = rect; bestRotated = false }
      }
      if (ph <= rect.w && pw <= rect.h) {
        const s1 = Math.min(rect.w - ph, rect.h - pw), s2 = Math.max(rect.w - ph, rect.h - pw)
        if (s1 < bestScore1 || (s1 === bestScore1 && s2 < bestScore2)) { bestScore1 = s1; bestScore2 = s2; bestRect = rect; bestRotated = true }
      }
    }
...
function validateSheet(placed, W, H) {
  ...
  for (const p of placed) {
    if (p.x + p.pw > W + 1) errors.push(`Peça ultrapassa largura: ${p.x + p.pw} > ${W}`)
    if (p.y + p.ph > H + 1) errors.push(`Peça ultrapassa altura: ${p.y + p.ph} > ${H}`)
  }
  for (let i = 0; i < placed.length; i++)
    for (let j = i + 1; j < placed.length; j++) {
      const a = placed[i], b = placed[j]
      if (rectsOverlap(a.x, a.y, a.pw, a.ph, b.x, b.y, b.pw, b.ph))
        errors.push(`Sobreposição entre peças ${i + 1} e ${j + 1}`)
    }
  return errors
}
...
// ══════════════════════════════════════════════════════════
// COMPONENTE: LAYOUT SVG DA CHAPA
// ══════════════════════════════════════════════════════════
  ...
  return (
    <svg width={maxW} height={svgH} style={{ display: "block", borderRadius: 8, background: "#141A14" }}>
        ...
        return (
          <g key={`f${i}`}>
            <rect x={sx(r.x)} y={sy(r.y)} width={rw} height={rh}
              fill="#2D3B2D" stroke="#4B5563" strokeWidth={1} strokeDasharray="5 3" opacity={0.85} />
            {rw > 40 && rh > 28 && (
              <text x={sx(r.x) + rw / 2} y={sy(r.y) + rh / 2}
                textAnchor="middle" dominantBaseline="middle"
                fontSize={fontSize(Math.min(rw, rh) * 0.14)} fill="#6B7280" fontFamily="monospace" fontWeight="600">
      ...
      {sheet.pieces.map((p, i) => {
        ...
        const finW = p.ref?.origW ?? p.pw - 4
        const finH = p.ref?.origH ?? p.ph - 4
        const fs = fontSize(Math.min(pw, ph) * 0.17)
        const fsS = Math.max(6, fs * 0.7)
        ...
        return (
          <g key={`p${i}`}>
            ...
            {showSize && (
              <>
                {/* Tamanho de CORTE — o que o operador corta */}
                <text x={midX} y={midY - (ph > 38 ? fsS + 1 : 0)}
                  textAnchor="middle" dominantBaseline="middle"
                  fontSize={fs} fill="#15803D" fontFamily="monospace" fontWeight="800">
                  {cutW}×{cutH}
                </text>
                {/* Tamanho FINAL — o que entrega ao cliente */}
                {ph > 38 && (
                  <text x={midX} y={midY + fs + 1}
                    textAnchor="middle" dominantBaseline="middle"
                    fontSize={fsS} fill="#166534" fontFamily="monospace" opacity={0.7}>
                    ({finW}×{finH})
                  </text>
                )}
                {p.rotated && ph > 52 && (
                  <text x={midX} y={midY + fs + fsS + 4}
                    textAnchor="middle" dominantBaseline="middle"
                    fontSize={fsS} fill="#22C55E" opacity={0.8}>↺</text>
                )}
              </>
            )}
          </g>
      ...
      <text x={maxW / 2} y={svgH - 5} textAnchor="middle" fontSize="9" fill="#6B7280" fontFamily="monospace">
...
function CutSequence({ sheet }) {
  ...
  sheet.pieces.forEach(p => {
    ...
    if (p.x + p.pw < sheet.width) xCuts.add(p.x + p.pw)
    if (p.y > 0) yCuts.add(p.y)
    if (p.y + p.ph < sheet.height) yCuts.add(p.y + p.ph)
  })
  const steps = [
    ...[...xCuts].sort((a, b) => a - b).map(x => ({ tipo: "Corte vertical", val: `${x} mm` })),
    ...[...yCuts].sort((a, b) => a - b).map(y => ({ tipo: "Corte horizontal", val: `${y} mm` })),
    ...sheet.pieces.map(p => ({ tipo: "Separar peça", val: `${p.ref?.origW ?? p.pw - 4}×${p.ref?.origH ?? p.ph - 4} mm` })),
  ]
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
      {steps.map((c, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 22, height: 22, borderRadius: "50%", background: T.green, color: "#fff", fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{i + 1}</div>
          <span style={{ fontSize: 13, color: T.textMid, flex: 1 }}>{c.tipo}</span>
          <span style={{ fontSize: 13, fontWeight: 600, color: T.green, fontFamily: "monospace" }}>{c.val}</span>
        </div>
      ))}
      {steps.length === 0 && <div style={{ color: T.textMuted, fontSize: 13 }}>Sem cortes.</div>}
    </div>
  )
}
...
// ══════════════════════════════════════════════════════════
const Pill = ({ cor }) => (
  <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: COR[cor]?.bg, padding: "2px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600, color: T.text }}>
    <span style={{ width: 8, height: 8, borderRadius: "50%", background: COR[cor]?.dot, flexShrink: 0 }} />
    {COR[cor]?.label}
  </span>
)
...
const Btn = ({ children, onClick, variant = "primary", size = "md", fullWidth, icon, disabled, style }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      style={{ ...vs[variant], padding: ps[size], fontSize: fs[size], borderRadius: 12, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, width: fullWidth ? "100%" : "auto", transition: "all .15s", ...style }}>
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: T.textMuted }}>8mm · Gerenciar chapas disponíveis</div>
        </div>
      ...
      {showAdd && (
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 20, marginBottom: 20, boxShadow: "0 4px 24px rgba(0,0,0,0.12)" }}>
          ...
          {[["largura", "Largura (mm)", "Ex: 2200"], ["altura", "Altura (mm)", "Ex: 3210"], ["quantidade", "Quantidade", "Ex: 10"]].map(([k, label, ph]) => (
          ...
          <div key={cor} style={{ marginBottom: 24 }}>
            ...
            {items.map(c => (
              <div key={c.id} style={{ background: T.card, borderRadius: 14, padding: "14px 16px", marginBottom: 8, display: "flex", alignItems: "center", gap: 12, boxShadow: "0 1px 3px rgba(0,0,0,0.07)" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "monospace" }}>{c.largura} × {c.altura} mm</div>
      ...
      {data.chapas.length === 0 && (
        <div style={{ textAlign: "center", padding: "48px 24px", color: T.textMuted }}>
          ...
          <div style={{ fontSize: 13 }}>Clique em Adicionar para cadastrar chapas</div>
        </div>
      )}
    </div>
  )
...
// NOVA OTIMIZAÇÃO
// ══════════════════════════════════════════════════════════
function NewOptimization({ data, setData, navigate }) {
  const [step, setStep] = useState(1)
  const [cor, setCor] = useState("incolor")
  const [pecas, setPecas] = useState([{ id: uid(), largura: "", altura: "", quantidade: 1 }])
  ...
  const handleOptimize = () => {
    ...
    setTimeout(() => {
      ...
      if (!chapa) { setError(`Sem chapas de ${COR[cor]?.label} em estoque. Adicione chapas primeiro.`); setLoading(false); return }
  ...
  const handleFinalize = async () => {
    ...
    const novosRetalhos = result.map(s => s.mainScrap).filter(Boolean).map(r => ({
      id: uid(), cor, largura: Math.round(r.w), altura: Math.round(r.h),
      area: parseFloat(area_m2(r.w, r.h)), origem: id, status: "ativo",
    }))

    const novaOtm = {
      id, cor, chapas_usadas: chapasUsadas,
      aproveitamento: Math.round((result.reduce((s, r) => s + r.efficiency, 0) / result.length) * 10) / 10,
      desperdicio: Math.round((100 - avgEff) * 10) / 10,
      pecas_totais: totalPecas, area_total: parseFloat((areaTotal / 1e6).toFixed(2)),
    }

    try {
      // Atualiza estoque de chapas no banco
      for (const c of data.chapas) {
        if (c.cor === cor) {
          const novaQtd = Math.max(0, c.quantidade - chapasUsadas)
          await DB.chapas.update(c.id, { quantidade: novaQtd })
        }
      }
      ...
      const otmLocal = { ...novaOtm, chapasUsadas: novaOtm.chapas_usadas, pecasTotais: novaOtm.pecas_totais, areaTotal: novaOtm.area_total, criadoEm: Date.now() }
      setData(d => ({
        ...d,
        chapas: novasChapas,
        retalhos: [...d.retalhos, ...novosRetalhos.map(r => ({ ...r, criado_em: Date.now() }))],
        otimizacoes: [otmLocal, ...d.otimizacoes],
      }))
      navigate("history")
    ...
    <div>
      ...
      {step === 1 && (
        <div>
            ...
            {["incolor", "verde", "fume"].map(c => {
              ...
              return (
                <button key={c} onClick={() => setCor(c)}
                  style={{ padding: "16px 8px", borderRadius: 14, border: `2px solid ${cor === c ? T.green : T.border}`, background: cor === c ? T.greenLight : "#fff", cursor: "pointer", textAlign: "center" }}>
                  <div style={{ width: 36, height: 36, borderRadius: 6, background: COR[c]?.bg, border: `2px solid ${COR[c]?.stroke}`, margin: "0 auto 8px" }} />
                  <div style={{ fontSize: 13, fontWeight: 700, color: T.text }}>{COR[c]?.label}</div>
                  <div style={{ fontSize: 11, color: stock > 0 ? T.green : T.red, fontWeight: 600 }}>{stock} chapas</div>
                </button>
              )
            })}
          </div>
          <div style={{ background: T.greenLight, borderRadius: 12, padding: "12px 16px", marginBottom: 24, display: "flex", gap: 10 }}>
            ...
            <div style={{ fontSize: 13, color: T.greenDark }}><strong>+2mm por lado (+4mm total)</strong> aplicado automaticamente em cada peça.</div>
          </div>
      ...
      {step === 2 && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 72px 36px", gap: 8, padding: "0 2px", marginBottom: 6 }}>
            {["Largura (mm)", "Altura (mm)", "Qtd.", ""].map((h, i) => <div key={i} style={{ fontSize: 11, fontWeight: 700, color: T.textMuted }}>{h}</div>)}
          </div>
          {pecas.map(p => (
            <div key={p.id} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 72px 36px", gap: 8, marginBottom: 8, alignItems: "center" }}>
              {["largura", "altura"].map(field => (
                <input key={field} type="number" value={p[field]} onChange={e => updatePeca(p.id, field, e.target.value)} placeholder={field === "largura" ? "500" : "980"}
                  style={{ padding: "10px 10px", borderRadius: 8, border: `1.5px solid ${T.border}`, fontSize: 14, outline: "none", background: "#F9FAFB", width: "100%", boxSizing: "border-box" }} />
              ))}
              <input type="number" value={p.quantidade} min={1} onChange={e => updatePeca(p.id, "quantidade", Math.max(1, parseInt(e.target.value) || 1))}
      ...
      {step === 3 && (
        <div>
          ...
          <div style={{ background: T.card, borderRadius: 16, padding: 20, marginBottom: 20, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
            ...
            {pecas.filter(p => int(p.largura) && int(p.altura)).map((p, i) => (
              <div key={p.id} style={{ display: "grid", gridTemplateColumns: "24px 1fr 1fr 44px", gap: 6, paddingBottom: 8, borderBottom: `1px solid ${T.border}`, marginBottom: 8 }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: T.green }}>{p.quantidade}×</div>
              </div>
            ))}
          </div>
          ...
          <Btn onClick={handleOptimize} fullWidth size="lg" disabled={loading} icon={loading ? <RefreshCw size={18} style={{ animation: "spin 1s linear infinite" }} /> : <Zap size={18} />}>
          ...
          <div>
            <div style={{ background: T.greenLight, borderRadius: 14, padding: "12px 16px", marginBottom: 16, display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: T.greenDark }}>{result.length} chapa(s) · {result.reduce((t, sh) => t + sh.pieces.length, 0)} peças distribuídas · {sheetInfo?.largura}×{sheetInfo?.altura} mm</div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14, overflowX: "auto", paddingBottom: 4 }}>
              {result.map((sh, i) => (
                <button key={i} onClick={() => setCurrentSheet(i)}
                  style={{ padding: "8px 14px", borderRadius: 10, border: `2px solid ${currentSheet === i ? T.green : T.border}`, background: currentSheet === i ? T.green : T.card, color: currentSheet === i ? "#fff" : T.textMid, fontSize: 13, fontWeight: 700, cursor: "pointer", flexShrink: 0, minWidth: 80, textAlign: "center" }}>
                  Chapa {i + 1}
            ...
            <div style={{ background: T.dark, borderRadius: 14, padding: "14px 14px", marginBottom: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 12 }}>
                {[["Peças", s.pieces.length, "#fff"], ["Aproveit.", `${s.efficiency}%`, T.green], ["Desperdício", `${(100 - s.efficiency).toFixed(1)}%`, "#FCA5A5"], ["Retalhos", s.freeRects.length, "#9CA3AF"]].map(([l, v, c]) => (
              ...
              <div style={{ overflowX: "auto" }}>
                <SheetSVG sheet={s} maxW={Math.min(380, window.innerWidth - 60)} />
              </div>
              <div style={{ display: "flex", gap: 16, marginTop: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#9CA3AF" }}>
                  <div style={{ width: 14, height: 10, background: "#DCFCE7", border: "1.5px solid #16A34A", borderRadius: 2 }} /> Peça cortada
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#9CA3AF" }}>
                  <div style={{ width: 14, height: 10, background: "#2D3B2D", border: "1.5px dashed #4B5563", borderRadius: 2 }} /> Sobra / Borda
                </div>
              </div>
            </div>
            ...
            {s.mainScrap && (
              <div style={{ background: T.card, borderRadius: 14, padding: 16, marginBottom: 16, boxShadow: "0 1px 3px rgba(0,0,0,0.07)" }}>
                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>Maior retalho — Chapa {currentSheet + 1}</div>
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  <div style={{ width: 44, height: 54, background: T.greenLight, border: `2px dashed ${T.green}`, borderRadius: 6, flexShrink: 0 }} />
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "monospace" }}>{Math.round(s.mainScrap.w)} × {Math.round(s.mainScrap.h)} mm</div>
                    <div style={{ fontSize: 12, color: T.textMuted }}>Área: {area_m2(s.mainScrap.w, s.mainScrap.h)} m²</div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              {!cortadas.has(currentSheet) && (
                <Btn onClick={() => setCortadas(prev => new Set([...prev, currentSheet]))} variant="secondary" size="sm" icon={<Check size={16} />} style={{ flex: 1 }}>Marcar cortada</Btn>
              )}
              {currentSheet < result.length - 1
                ? <Btn onClick={() => setCurrentSheet(i => i + 1)} size="sm" icon={<ChevronRight size={16} />} style={{ flex: 1 }}>Próxima chapa</Btn>
...
function ScrapsScreen({ data, setData }) {
  ...
  const RetalhoCard = ({ r }) => (
    <div style={{ background: T.card, borderRadius: 14, padding: "14px 16px", marginBottom: 8, display: "flex", gap: 12, alignItems: "center", boxShadow: "0 1px 3px rgba(0,0,0,0.07)" }}>
      <div style={{ width: 40, height: 50, background: COR[r.cor]?.piece || T.greenLight, border: `2px dashed ${COR[r.cor]?.stroke || T.green}`, borderRadius: 6, flexShrink: 0 }} />
      ...
      {r.status === "ativo"
        ? <button onClick={() => updateStatus(r.id, "descartado")} style={{ background: T.redLight, border: "none", borderRadius: 8, padding: "6px 10px", cursor: "pointer", display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: T.red, fontWeight: 600 }}><Trash2 size={13} /> Descartar</button>
        : <button onClick={() => updateStatus(r.id, "ativo")} style={{ background: T.greenLight, border: "none", borderRadius: 8, padding: "6px 10px", cursor: "pointer", display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: T.greenDark, fontWeight: 600 }}><RotateCcw size={13} /> Restaurar</button>
      }
    </div>
  )
...
function HistoryScreen({ data }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 13, color: T.textMuted, marginBottom: 20 }}>Todas as otimizações realizadas</div>
      {data.otimizacoes.map(o => (
        <div key={o.id} style={{ background: T.card, borderRadius: 16, padding: "16px 18px", marginBottom: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
...
export default function App() {
  ...
  if (data.loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: T.dark, flexDirection: "column", gap: 16 }}>
      <div style={{ fontSize: 30, fontWeight: 800, color: T.green }}>UTM<span style={{ color: "#fff" }}>glass</span></div>
  ...
  if (dbError) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: T.dark, flexDirection: "column", gap: 16, padding: 32 }}>
      ...
      <div style={{ color: "#fff", fontSize: 18, fontWeight: 700, textAlign: "center" }}>Erro de conexão com o banco</div>
      <div style={{ color: "#9CA3AF", fontSize: 13, textAlign: "center" }}>Verifique o arquivo .env com as credenciais do Supabase</div>
  ...
  const props = { data, setData, navigate: setScreen }
    ...
    <div style={{ maxWidth: 560, margin: "0 auto", minHeight: "100vh", background: T.bg, fontFamily: "'Barlow', sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: T.dark, padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {screen !== "dashboard"
            ? <ArrowLeft size={20} color="#9CA3AF" style={{ cursor: "pointer" }} onClick={() => setScreen("dashboard")} />
            : <MoreVertical size={20} color="#9CA3AF" style={{ cursor: "pointer" }} onClick={() => setMenuOpen(true)} />
          }
          <span style={{ fontSize: 22, fontWeight: 800, color: "#fff", letterSpacing: -0.5 }}>UTM<span style={{ color: T.green }}>glass</span></span>
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex" }}>
          <div style={{ background: T.dark, width: 280, height: "100%", display: "flex", flexDirection: "column", boxShadow: "4px 0 24px rgba(0,0,0,.5)" }}>
            <div style={{ padding: "24px 24px 20px", borderBottom: "1px solid #1F2D1F", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontSize: 20, fontWeight: 800, color: "#fff" }}>UTM<span style={{ color: T.green }}>glass</span></div>
            ...
            {NAV.map(n => (
              <button key={n.id} onClick={() => { setScreen(n.id); setMenuOpen(false) }}
            ...
            <div style={{ marginTop: "auto", padding: "16px 24px", borderTop: "1px solid #1F2D1F", fontSize: 11, color: "#4B5563", textAlign: "center" }}>
              UTMglass v1.1.0 · Supabase + Vercel
            </div>
          </div>
      ...
      <div style={{ flex: 1, padding: "20px 20px 100px", overflowY: "auto" }}>
        ...
        {screen === "new-opt"   && <NewOptimization key="new-opt" {...props} />}
